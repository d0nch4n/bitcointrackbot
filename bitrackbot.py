...
import time  # Importato per registrare il timestamp
...
def init_db():
    try:
        ...
        c = conn.cursor()
        # Modificato per includere il campo timestamp
...
async def set_send_address(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    timestamp = int(time.time())  # Registra il timestamp corrente
    conn = sqlite3.connect('subscriptions.db')
    conn.execute(f"PRAGMA key = '{DB_KEY}'")
    c = conn.cursor()
...
# Monitoraggio indirizzi (aggiornato per filtrare transazioni dopo il timestamp)
    ...
    for user_id, address, sub_type, activation_timestamp in subscriptions:
        ...
        for tx in txs:
            ...
            if tx_details and tx_details.get('status', {}).get('confirmed', False):
                ...
                # Filtra transazioni avvenute prima dell'attivazione del monitoraggio
...
# Monitoraggio transazioni (aggiornato per filtrare transazioni dopo il timestamp)
